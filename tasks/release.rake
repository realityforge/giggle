...
### [v#{ENV['PRODUCT_VERSION']}](https://github.com/realityforge/giggle/tree/v#{ENV['PRODUCT_VERSION']}) (#{ENV['RELEASE_DATE']}) · [Full Changelog](https://github.com/realityforge/giggle/compare/#{from}...v#{ENV['PRODUCT_VERSION']})
HEADER
...
HEADER
      ...
      `bundle exec zapwhite --exclude-pattern 'compiler/src/test/fixtures/[^/]+/output/.*'`
      sh 'git add CHANGELOG.md'
    ...
    stage('GithubRelease', 'Create a Release on GitHub') do
      ...
      client.login
      client.create_release('realityforge/giggle', tag, :name => tag, :body => changes, :draft => false, :prerelease => prerelease)

      candidates = client.list_milestones('realityforge/giggle').select {|m| m[:title].to_s == tag}
      unless candidates.empty?
        milestone = candidates[0]
        unless milestone[:state] == 'closed'
          client.update_milestone('realityforge/giggle', milestone[:number], :state => 'closed')
        end
      end
    end
  end
